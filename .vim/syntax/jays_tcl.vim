" Vim syntax file
" Language:	TCL/TK
" Maintainer:	Updated by: Jay Taylor <outtatime@gmail.com>
...
syn keyword tclStatement	set
syn keyword tclStatement 	catch unset
...
syn region tcltkCommand matchgroup=tcltkCommandColor start="\<reg\(sub\|exp\)\>" matchgroup=NONE skip="^\s*$" end="]\|[^\\]*\s*$"he=e-1  contains=tcltkRegSwitch,tcltkRegSwitchA,tcltkRegSwitchB,tclLineContinue,tcltkRegCommand,tclNumber,tclVarRef,tclString,tcltkCommand

" INFO
"
syn keyword tcltkInfoSwitch contained args body cmdcount commands complete default exists globals hostname level library loaded locals nameofexecutable patchlevel procs script sharedlibextension tclversion vars
syn region tcltkCommand matchgroup=tcltkCommandColor start="\<info\>" matchgroup=NONE skip="^\s*$" end="]\|[^\\]*\s*$"he=e-1 contains=tcltkInfoSwitch

" CLOCK
"
syn keyword tcltkClockSwitch contained exists
syn region tcltkCommand matchgroup=tcltkCommandColor start="\<clock\>" matchgroup=NONE skip="^\s*$" end="]\|[^\\]*\s*$"he=e-1 contains=tcltkClockSwitch
...
if version >= 508 || !exists("did_tcl_syntax_inits")
  ...
  HiLink tcltkLfuckSwitch	Special
